...
@section('content')
    ...
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Обновить адрес</h2>
            <a href="{{ route('admin.address.index') }}" class="text-gray-500 hover:text-gray-700 flex items-center">
        ...
        <div class="bg-white shadow-md rounded-lg p-8">
            <form action="{{ route('admin.address.update', $address->getSlug()) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
                @csrf
                @method('patch')

                <div>
                    <label class="block text-sm font-medium text-gray-700">Название категории</label>
                    <input type="text" name="name" value="{{ $address->getName() }}" required
