        ...
        <div class="bg-white shadow-md rounded-lg p-8">
            <form action="{{ route('admin.product.update', $product) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <textarea required name="description" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border @error('description') border-red-500 @enderror">{{ $product->description }}</textarea>
                ...
                <div>
                    @if($product->hasMedia('products'))
                        ...
                        <input type="file" name="product_image" accept="image/jpeg,image/png,image/jpg" value="{{ old('product_image') }}"
                    ...
                    <div id="specs-wrapper" class="space-y-3">
                        @foreach($product->getSpecs() as $key => $value)
                            <div class="row flex items-center gap-3 animate-fadeIn">
                                ...
                                <div class="flex-1">
                                    <input type="text" name="specs_values[]" value="{{ $value }}" placeholder="Значение" required
                                           class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border text-sm">
                                </div>
