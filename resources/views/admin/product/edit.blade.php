    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-md rounded-lg p-8"
             x-data="productEditForm({
                ...
                currentCategoryId: {{ old('category_id', $product->category_id ?? 0) }},
                currentSubCategoryId: {{ old('sub_category_id', $product->sub_category_id ?? 0) }},
                ...
                // Подгружаем уже сохраненные склады товара
                savedAddresses: {{ json_encode($product->addresses->map(fn($a) => ['address_id' => $a->id, 'product_quantity' => $a->pivot->product_quantity])) }},
                // Подгружаем уже сохраненные характеристики товара
                savedSpecs: {{ json_encode($product->propertyValues->map(fn($pv) => ['property_id' => $pv->property_id, 'value' => $pv->value])) }}
             })">

            <form action="{{ route('admin.product.update', $product) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <textarea required name="description" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border @error('description') border-red-500 @enderror">{{ old('description', $product->description) }}</textarea>
                                ...
                                <div class="flex-1">
                                    <select name="address_ids[]" x-model="item.address_id" required
                                        ...
                                        <template x-for="addr in allAddresses" :key="addr.id">
                                            <option :value="addr.id" x-text="addr.name" :selected="addr.id == item.address_id"></option>
                                        </template>
                                    </select>
                                </div>
                                <div class="flex-1">
                ...
                <div class="border-t pt-4">
                    ...
                    @if($product->hasMedia('products'))
                        <div class="mt-2">
                            <p class="text-xs text-gray-500 mb-1">Текущее изображение:</p>
                            <img src="{{ $product->getFirstMediaUrl('products') }}" class="w-32 h-32 object-cover rounded border shadow-sm">
                        </div>
                    @endif
                </div>
                                ...
                                <div class="flex-1">
                                    <select name="properties[]" x-model="spec.property_id" required
                                        ...
                                        <template x-for="prop in allProperties" :key="prop.id">
                                            <option :value="prop.id" x-text="prop.name" :selected="prop.id == spec.property_id"></option>
                                        </template>
                                    </select>
                                </div>
                                <div class="flex-1">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('productEditForm', (config) => ({
            ...
            selectedAddresses: config.savedAddresses.length ? config.savedAddresses : [{ address_id: '', product_quantity: 0 }],

            allProperties: config.allProperties,
            selectedSpecs: config.savedSpecs.length ? config.savedSpecs : [{ property_id: '', value: '' }],

            init() {
                // Подгружаем подкатегории для текущей категории при загрузке страницы
                this.updateSubCategories();
            },

            onCategoryChange() {
                this.updateSubCategories();
                this.subCategoryId = 0; // Сбрасываем подкатегорию при смене основной категории
            },
            ...
            // Логика управления характеристиками
            addSpec() {
