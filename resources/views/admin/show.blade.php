    ...
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-gray-500">ID пользователя: #{{ $user->getKey() }}</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('admin.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 transition">
                    К списку
                </a>
                <a href="{{ route('admin.edit', $user->getKey()) }}" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition">
                    Редактировать
                </a>
            </div>
        </div>
            ...
            <div class="p-6">
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-8">
                    <div class="sm:col-span-1">
                        ...
                        <dd class="mt-1 text-lg text-gray-900 font-semibold">{{ $user->getEmail() }}</dd>
                    </div>

                    <div class="sm:col-span-1">
                        <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">Номер телефона</dt>
                        <dd class="mt-1 text-lg text-gray-900 font-semibold">{{ $user->getPhone() ?? 'Не указан' }}</dd>
                    </div>
