...
@section('content')
    ...
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.subCategory.index') }}" class="text-gray-500 hover:text-gray-700 flex items-center">
        ...
        <div class="bg-white shadow-md rounded-lg p-8">
            <form action="{{ route('admin.subCategory.update', $subCategory->getSlug()) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
                @csrf
                @method('patch')

                <div>
                    <label class="block text-sm font-medium text-gray-700">Название категории</label>
                    <input type="text" name="name" value="{{ $subCategory->getName() }}" required
                ...
                <div>
                    ...
                    <select name="category_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border">
                        <option value="{{ $subCategory->getCategory()->getKey() }}">{{ $subCategory->getCategory()->getName() }}</option>
                ...
                <div>
                    @if($subCategory->hasMedia('subCategory_images'))
                        ...
                        <img class="h-20" src="{{ $subCategory->getFirstMediaUrl('subCategory_images') }}" name="category_image" alt="">
