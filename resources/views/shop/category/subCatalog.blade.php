...
@section('content')
    <div class="max-w-[1600px] mx-auto px-6 py-12">
        ...
        <nav class="flex mb-6 text-sm text-gray-500 dark:text-gray-400">
            <a href="{{ route('catalog.index') }}" class="hover:text-indigo-600 transition">Каталог</a>
            <span class="mx-2">/</span>
            <span class="text-gray-900 dark:text-white font-medium">{{ $category->getName() }}</span>
        </nav>
        ...
        @if($category->getSubCategories()->isEmpty())
            <div class="text-center py-20 bg-gray-50 dark:bg-gray-900 rounded-3xl">
                <p class="text-xl text-gray-500 dark:text-gray-400">В этой категории пока нет подкатегорий...</p>
            </div>
        @else
                    ...
                    <div class="group bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300">
                        <a href="{{ route('catalog.product', $subCategory->getSlug()) }}" class="block">
                            ...
                            <div class="p-5">
                                ...
                                <span class="inline-flex items-center text-indigo-600 dark:text-indigo-400 font-bold text-sm">
                                    Перейти
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        @can('delete-categories')
                                            <form method="POST" action="{{ route('admin.subCategory.destroy', $subCategory->getKey()) }}"
                                                  onsubmit="return confirm('Удалить подкатегорию? Все товары в ней тоже удалятся?')">
                                                @csrf
                                                @method('delete')
                                                <button class="text-red-600 dark:text-red-400 hover:text-red-800 text-sm font-medium">Удалить</button>
                                            </form>
                                        @endcan
                                    </div>
                                </div>
